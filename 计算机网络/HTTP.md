...
GET 方法只是可读的，因此它是安全的，而 POST 却不是，因为 POST 的目的是传送实体主体内容，这个内容可能是用户上传的表单数据，上传成功之后，服务器可能把这个数据存储到数据库中，因此状态也就发生了改变。
...
幂等的 HTTP 方法，同样的请求被执行一次与连续执行多次的效果是一样的，服务器的状态也是一样的。换句话说就是，幂等方法不应该具有副作用（**请注意，这里强调的是一次和N次具有相同的副作用，而不是每次GET的结果相同**）。
...
比如：POST http://www.forum.com/articles 的语义是在 http://www.forum.com/articles 下创建一篇帖子，两次相同的 POST 请求会在服务器端创建两份资源，它们具有不同的 URI；所以，POST 方法不具备幂等性。而PUT 所对应的 URI 是要创建或更新的资源本身。比如：PUT http://www.forum/articles/4231 的语义是创建或更新 ID 为4231的帖子。对同一个 URI 进行多次 PUT 的副作用和一次 PUT 是相同的；因此，PUT 方法具有幂等性。
...
- 个性化设置（如用户自定义设置、主题等）
- 浏览器行为跟踪（如跟踪分析用户行为等，可能会给用户的隐私带来威胁）
...
客户端之后对**同一个服务器**发送请求时，会从浏览器中取出 Cookie 信息并通过 Cookie 请求首部字段发送给服务器。
...
- 会话期 Cookie：浏览器关闭之后它会被自动删除，也就是说它仅在会话期内有效 (在会话期间该 Cookie 保存在浏览器的缓存中)。
...
private 指令规定了将资源作为私有缓存，只能被单独用户所使用，一般存储在**用户浏览器**中。

```html
Cache-Control: private
```

public 指令规定了将资源作为公共缓存，可以被多个用户所使用，一般存储在**代理服务器**中。
...
1. 服务器将自己的公钥发送给客户端。
2. 客户端在本地生成一个 SESSION KEY。
3. 客户端使用公钥对 SESSION KEY 进行加密并发送给服务器。
4. 服务器使用私钥对 SESSION KEY 进行解密。这样 SESSION KEY 就通过非对称密钥加密的方式从客户端发送到了服务器。
5. 客户端和服务器将该 SESSION KEY 作为公钥，使用对称密钥加密的方式进行通信。
...
1、以前客户端需要使用多个连接才能实现并发，在 HTTP/2.0 中由于一个 TCP 连接上可以承载多个数据流，一个数据流上就能承载通信双方的信息，因此客户端现在实现并发可能只需要一个连接就够了。

2、由于数据流上有可选的优先级信息，因此可以支持资源的优先级。
