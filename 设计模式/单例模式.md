...
该实现在多次序列化再进行反序列化之后，不会得到多个实例。而其它实现需要使用 transient 修饰所有字段 (transient  修饰的字段在对象序列化的时候不会进行序列化)，并且实现序列化和反序列化的方法。
...
[单例模式](https://cyc2018.github.io/CS-Notes/#/notes/%E8%AE%BE%E8%AE%A1%E6%A8%A1%E5%BC%8F)
